...
Folder Sort Z to A changes only the rendered order of folder items in the native File Explorer. It does not create, modify, delete, rename, or transmit vault files. It does not call git, use network APIs, or access the clipboard. The implementation depends on Obsidian File Explorer internals and should fail closed with a warning if those internals change.
